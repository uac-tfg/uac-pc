...
/**
 * Represents a digitally sampled wave. Uses a file as a buffer to save the
 * samples to in .wav format.
 * 
...
public class Wave {
	 ...
	 * 
	 * @param file
	 *            the file to use as a buffer
	 * @param config
	 *            the config to use
	 */
	public Wave(File file, WaveConfig config) {
		try {
			this.wav = RandomAccessWavFile.newWavFile(file, 1, config.getNumFrames(), config.getValidBits(),
	...
	/**
	 * Adds a wave to the local wave. In the process of adding, interference is
	 * simulated. This means that every sample is added one by one. If the value
	 * gets higher than or lower than 1 or -1, the frame can't be saved correct
	 * because of a overflow.
	 * 
	 * @param wave
	 *            the wave to add
	 * @param toStart
	 *            the starting position in this wave
	 * @param fromStart
	 *            the starting position in the given wave
	 * @param length
	 *            the number of frames to add
	 * @param a
	 *            scale factor, set to 1 for no effect
	 */
